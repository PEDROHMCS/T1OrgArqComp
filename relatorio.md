...
### Pablo Vieira - 16895429
Minha colaboração envolveu as operações de inserção de novos vagões (tanto no início quanto no final do trem) e a função de exibição da lista completa. A inserção exigiu o desenvolvimento de laços de repetição para percorrer a lista a partir da locomotiva, avaliando as condições de parada dinamicamente (identificando ponteiros nulos para inserção no fim, ou o nó adjacente à cabeça para o início) e reajustando os engates (o conteúdo dos ponteiros para os próximos vagões). A função de exibição reutilizou essa lógica para ler e imprimir os dados a partir da iteração na lista. O principal obstáculo enfrentado foi estruturar de forma segura a manipulação dos registradores dentro dos loops, além de consolidar o entendimento técnico sobre a diferença entre acessar um endereço na memória e manipular o valor armazenado nele.
